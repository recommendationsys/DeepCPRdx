...
def train(dataset, kg, model, bs, max_epoch, optimizer1, optimizer2, optimizer3, reg, qonly, observe, command, filename, uf, useremb, load_fm_epoch):
    ...
    for epoch in range(load_fm_epoch, max_epoch+1):
        ...
        if epoch % 5 == 0 and epoch > -1:            # 因为>0，所以没有保存embeds-epoch-0.pkl这一文件，需改成>-1
...
def main():
    ...
    # means quadratic form only (letting go other terms in FM equation...)           只表示二次项，抛开FM方程中的其他项
    ...
    parser.add_argument('--data_name', type=str, default='YELP_STAR', metavar='<data_name>', dest='data_name',
    ...
    PAD_IDX1 = user_length + item_length            # 用户和项目数量之和
    PAD_IDX2 = feature_length                       # 属性数量之和
